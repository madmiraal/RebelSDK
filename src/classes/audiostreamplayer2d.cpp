// SPDX-FileCopyrightText: 2025 Rebel SDK contributors
//
// SPDX-License-Identifier: MIT

#include "classes/audiostreamplayer2d.h"

#include "classes/audiostream.h"
#include "classes/audiostreamplayback.h"
#include "classes/icalls.h"
#include "common/rebel.h"
#include "common/rebelglobal.h"

namespace Rebel {
void AudioStreamPlayer2D::_bus_layout_changed() {
    rebel_icall_void(
        method_bindings.mb__bus_layout_changed,
        (const Object*)this
    );
}

bool AudioStreamPlayer2D::_is_active() const {
    return rebel_icall_bool(
        method_bindings.mb__is_active,
        (const Object*)this
    );
}

void AudioStreamPlayer2D::_set_playing(const bool enable) {
    rebel_icall_void_bool(
        method_bindings.mb__set_playing,
        (const Object*)this,
        enable
    );
}

int64_t AudioStreamPlayer2D::get_area_mask() const {
    return rebel_icall_int(
        method_bindings.mb_get_area_mask,
        (const Object*)this
    );
}

real_t AudioStreamPlayer2D::get_attenuation() const {
    return rebel_icall_float(
        method_bindings.mb_get_attenuation,
        (const Object*)this
    );
}

String AudioStreamPlayer2D::get_bus() const {
    return rebel_icall_string(
        method_bindings.mb_get_bus,
        (const Object*)this
    );
}

real_t AudioStreamPlayer2D::get_max_distance() const {
    return rebel_icall_float(
        method_bindings.mb_get_max_distance,
        (const Object*)this
    );
}

real_t AudioStreamPlayer2D::get_pitch_scale() const {
    return rebel_icall_float(
        method_bindings.mb_get_pitch_scale,
        (const Object*)this
    );
}

real_t AudioStreamPlayer2D::get_playback_position() {
    return rebel_icall_float(
        method_bindings.mb_get_playback_position,
        (const Object*)this
    );
}

Ref<AudioStream> AudioStreamPlayer2D::get_stream() const {
    return Ref<AudioStream>::create_ref(rebel_icall_object(
        method_bindings.mb_get_stream,
        (const Object*)this
    ));
}

bool AudioStreamPlayer2D::get_stream_paused() const {
    return rebel_icall_bool(
        method_bindings.mb_get_stream_paused,
        (const Object*)this
    );
}

Ref<AudioStreamPlayback> AudioStreamPlayer2D::get_stream_playback() {
    return Ref<AudioStreamPlayback>::create_ref(rebel_icall_object(
        method_bindings.mb_get_stream_playback,
        (const Object*)this
    ));
}

real_t AudioStreamPlayer2D::get_volume_db() const {
    return rebel_icall_float(
        method_bindings.mb_get_volume_db,
        (const Object*)this
    );
}

bool AudioStreamPlayer2D::is_autoplay_enabled() {
    return rebel_icall_bool(
        method_bindings.mb_is_autoplay_enabled,
        (const Object*)this
    );
}

bool AudioStreamPlayer2D::is_playing() const {
    return rebel_icall_bool(
        method_bindings.mb_is_playing,
        (const Object*)this
    );
}

void AudioStreamPlayer2D::play(const real_t from_position) {
    rebel_icall_void_float(
        method_bindings.mb_play,
        (const Object*)this,
        from_position
    );
}

void AudioStreamPlayer2D::seek(const real_t to_position) {
    rebel_icall_void_float(
        method_bindings.mb_seek,
        (const Object*)this,
        to_position
    );
}

void AudioStreamPlayer2D::set_area_mask(const int64_t mask) {
    rebel_icall_void_int(
        method_bindings.mb_set_area_mask,
        (const Object*)this,
        mask
    );
}

void AudioStreamPlayer2D::set_attenuation(const real_t curve) {
    rebel_icall_void_float(
        method_bindings.mb_set_attenuation,
        (const Object*)this,
        curve
    );
}

void AudioStreamPlayer2D::set_autoplay(const bool enable) {
    rebel_icall_void_bool(
        method_bindings.mb_set_autoplay,
        (const Object*)this,
        enable
    );
}

void AudioStreamPlayer2D::set_bus(const String bus) {
    rebel_icall_void_string(
        method_bindings.mb_set_bus,
        (const Object*)this,
        bus
    );
}

void AudioStreamPlayer2D::set_max_distance(const real_t pixels) {
    rebel_icall_void_float(
        method_bindings.mb_set_max_distance,
        (const Object*)this,
        pixels
    );
}

void AudioStreamPlayer2D::set_pitch_scale(const real_t pitch_scale) {
    rebel_icall_void_float(
        method_bindings.mb_set_pitch_scale,
        (const Object*)this,
        pitch_scale
    );
}

void AudioStreamPlayer2D::set_stream(const Ref<AudioStream> stream) {
    rebel_icall_void_object(
        method_bindings.mb_set_stream,
        (const Object*)this,
        stream.ptr()
    );
}

void AudioStreamPlayer2D::set_stream_paused(const bool pause) {
    rebel_icall_void_bool(
        method_bindings.mb_set_stream_paused,
        (const Object*)this,
        pause
    );
}

void AudioStreamPlayer2D::set_volume_db(const real_t volume_db) {
    rebel_icall_void_float(
        method_bindings.mb_set_volume_db,
        (const Object*)this,
        volume_db
    );
}

void AudioStreamPlayer2D::stop() {
    rebel_icall_void(
        method_bindings.mb_stop,
        (const Object*)this
    );
}

AudioStreamPlayer2D* AudioStreamPlayer2D::create() {
    return (AudioStreamPlayer2D*)nativescript_1_1_api->
        rebel_nativescript_get_instance_binding_data(
            RegisterState::language_index,
            api->rebel_get_class_constructor((char *)"AudioStreamPlayer2D")()
         );
}

void AudioStreamPlayer2D::init_method_bindings() {
    method_bindings.mb__bus_layout_changed =
        api->rebel_method_bind_get_method(
            "AudioStreamPlayer2D",
            "_bus_layout_changed"
        );
    method_bindings.mb__is_active =
        api->rebel_method_bind_get_method(
            "AudioStreamPlayer2D",
            "_is_active"
        );
    method_bindings.mb__set_playing =
        api->rebel_method_bind_get_method(
            "AudioStreamPlayer2D",
            "_set_playing"
        );
    method_bindings.mb_get_area_mask =
        api->rebel_method_bind_get_method(
            "AudioStreamPlayer2D",
            "get_area_mask"
        );
    method_bindings.mb_get_attenuation =
        api->rebel_method_bind_get_method(
            "AudioStreamPlayer2D",
            "get_attenuation"
        );
    method_bindings.mb_get_bus =
        api->rebel_method_bind_get_method(
            "AudioStreamPlayer2D",
            "get_bus"
        );
    method_bindings.mb_get_max_distance =
        api->rebel_method_bind_get_method(
            "AudioStreamPlayer2D",
            "get_max_distance"
        );
    method_bindings.mb_get_pitch_scale =
        api->rebel_method_bind_get_method(
            "AudioStreamPlayer2D",
            "get_pitch_scale"
        );
    method_bindings.mb_get_playback_position =
        api->rebel_method_bind_get_method(
            "AudioStreamPlayer2D",
            "get_playback_position"
        );
    method_bindings.mb_get_stream =
        api->rebel_method_bind_get_method(
            "AudioStreamPlayer2D",
            "get_stream"
        );
    method_bindings.mb_get_stream_paused =
        api->rebel_method_bind_get_method(
            "AudioStreamPlayer2D",
            "get_stream_paused"
        );
    method_bindings.mb_get_stream_playback =
        api->rebel_method_bind_get_method(
            "AudioStreamPlayer2D",
            "get_stream_playback"
        );
    method_bindings.mb_get_volume_db =
        api->rebel_method_bind_get_method(
            "AudioStreamPlayer2D",
            "get_volume_db"
        );
    method_bindings.mb_is_autoplay_enabled =
        api->rebel_method_bind_get_method(
            "AudioStreamPlayer2D",
            "is_autoplay_enabled"
        );
    method_bindings.mb_is_playing =
        api->rebel_method_bind_get_method(
            "AudioStreamPlayer2D",
            "is_playing"
        );
    method_bindings.mb_play =
        api->rebel_method_bind_get_method(
            "AudioStreamPlayer2D",
            "play"
        );
    method_bindings.mb_seek =
        api->rebel_method_bind_get_method(
            "AudioStreamPlayer2D",
            "seek"
        );
    method_bindings.mb_set_area_mask =
        api->rebel_method_bind_get_method(
            "AudioStreamPlayer2D",
            "set_area_mask"
        );
    method_bindings.mb_set_attenuation =
        api->rebel_method_bind_get_method(
            "AudioStreamPlayer2D",
            "set_attenuation"
        );
    method_bindings.mb_set_autoplay =
        api->rebel_method_bind_get_method(
            "AudioStreamPlayer2D",
            "set_autoplay"
        );
    method_bindings.mb_set_bus =
        api->rebel_method_bind_get_method(
            "AudioStreamPlayer2D",
            "set_bus"
        );
    method_bindings.mb_set_max_distance =
        api->rebel_method_bind_get_method(
            "AudioStreamPlayer2D",
            "set_max_distance"
        );
    method_bindings.mb_set_pitch_scale =
        api->rebel_method_bind_get_method(
            "AudioStreamPlayer2D",
            "set_pitch_scale"
        );
    method_bindings.mb_set_stream =
        api->rebel_method_bind_get_method(
            "AudioStreamPlayer2D",
            "set_stream"
        );
    method_bindings.mb_set_stream_paused =
        api->rebel_method_bind_get_method(
            "AudioStreamPlayer2D",
            "set_stream_paused"
        );
    method_bindings.mb_set_volume_db =
        api->rebel_method_bind_get_method(
            "AudioStreamPlayer2D",
            "set_volume_db"
        );
    method_bindings.mb_stop =
        api->rebel_method_bind_get_method(
            "AudioStreamPlayer2D",
            "stop"
        );

    rebel_string_name class_name;
    api->rebel_string_name_new_data(&class_name, "AudioStreamPlayer2D");
    class_tag = core_1_2_api->rebel_get_class_tag(&class_name);
    api->rebel_string_name_destroy(&class_name);
}

AudioStreamPlayer2D::MethodBindings AudioStreamPlayer2D::method_bindings = {};
void* AudioStreamPlayer2D::class_tag = nullptr;
} // namespace Rebel

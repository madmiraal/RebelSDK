...
bool Basis::isequal_approx(const Basis& a, const Basis& b) const {
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
            if ((::fabs(a.elements[i][j] - b.elements[i][j]) < CMP_EPSILON)
                == false) {
                return false;
            }
        }
...
Vector3 Basis::get_scale() const {
    // We are assuming M = R.S, and performing a polar decomposition to extract
    // R and S.
    // FIXME: We eventually need a proper polar decomposition.
    // As a cheap workaround until then, to ensure that R is a proper rotation
    // matrix with determinant +1 (such that it can be represented by a Quat or
    // Euler angles), we absorb the sign flip into the scaling matrix. As such,
    // it works in conjuction with get_rotation().
...
// TODO: implement this directly without using quaternions to make it more
// efficient
...
Basis::operator Quat() const {
    // commenting this check because precision issues cause it to fail when it
    // shouldn't ERR_FAIL_COND_V(is_rotation() == false, Quat());
    ...
    if (trace > 0.0) {
        ...
        real_t s =
            ::sqrt(elements[i][i] - elements[j][j] - elements[k][k] + 1.0);
        temp[i] = s * 0.5;
        s       = 0.5 / s;

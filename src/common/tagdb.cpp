...
void register_type(const size_t type_tag, const size_t base_type_tag) {
...
bool is_type_known(const size_t type_tag) {
    return parent_to.find(type_tag) != parent_to.end();
}

void register_global_type(
    const char* name,
    const size_t type_tag,
    const size_t base_type_tag
) {
...
bool is_type_compatible(const size_t from_tag, size_t to_tag) {
    if (to_tag == 0) {
        return false;
    }
    while (to_tag != 0) {

// SPDX-FileCopyrightText: 2025 Rebel SDK contributors
//
// SPDX-License-Identifier: MIT

#ifndef REBEL_MATERIAL_H
#define REBEL_MATERIAL_H

#include "api/gdnative_api_struct.gen.h"
#include "classes/resource.h"
#include "common/commontypes.h"
#include "common/ref.h"

#include <cstdint>

namespace Rebel {
class Material : public Resource {
public:
    static const int RENDER_PRIORITY_MAX = 127;
    static const int RENDER_PRIORITY_MIN = -128;

    Ref<Material> get_next_pass() const;
    int64_t get_render_priority() const;
    void set_next_pass(const Ref<Material> next_pass);
    void set_render_priority(const int64_t priority);

    static void init_method_bindings();

    inline static size_t get_class_tag() {
        return (size_t)class_tag;
    }

    inline static const char* _get_class_name() {
        return "Material";
    }

    inline static const char* get_rebel_class_name() {
        return "Material";
    }

    inline static Object* from_variant(Variant variant) {
        rebel_object* object = (rebel_object*)variant;
        if (!object) {
            return nullptr;
        }
        return (Object*)nativescript_1_1_api->
            rebel_nativescript_get_instance_binding_data(
                RegisterState::language_index,
                object
            );
    }

private:
    struct MethodBindings {
        rebel_method_bind* mb_get_next_pass;
        rebel_method_bind* mb_get_render_priority;
        rebel_method_bind* mb_set_next_pass;
        rebel_method_bind* mb_set_render_priority;
    };

    static MethodBindings method_bindings;
    static void* class_tag;
};
} // namespace Rebel

#endif // REBEL_MATERIAL_H

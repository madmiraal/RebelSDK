// SPDX-FileCopyrightText: 2025 Rebel SDK contributors
//
// SPDX-License-Identifier: MIT

#ifndef REBEL_BACKBUFFERCOPY_H
#define REBEL_BACKBUFFERCOPY_H

#include "api/gdnative_api_struct.gen.h"
#include "classes/node2d.h"
#include "common/commontypes.h"
#include "common/ref.h"

#include <cstdint>

namespace Rebel {
class BackBufferCopy : public Node2D {
public:
    enum CopyMode {
        COPY_MODE_DISABLED = 0,
        COPY_MODE_RECT = 1,
        COPY_MODE_VIEWPORT = 2,
    };

    BackBufferCopy::CopyMode get_copy_mode() const;
    Rect2 get_rect() const;
    void set_copy_mode(const int64_t copy_mode);
    void set_rect(const Rect2 rect);

    static BackBufferCopy* create();

    static void init_method_bindings();

    inline static size_t get_class_tag() {
        return (size_t)class_tag;
    }

    inline static const char* _get_class_name() {
        return "BackBufferCopy";
    }

    inline static const char* get_rebel_class_name() {
        return "BackBufferCopy";
    }

    inline static Object* from_variant(Variant variant) {
        rebel_object* object = (rebel_object*)variant;
        if (!object) {
            return nullptr;
        }
        return (Object*)nativescript_1_1_api->
            rebel_nativescript_get_instance_binding_data(
                RegisterState::language_index,
                object
            );
    }

private:
    struct MethodBindings {
        rebel_method_bind* mb_get_copy_mode;
        rebel_method_bind* mb_get_rect;
        rebel_method_bind* mb_set_copy_mode;
        rebel_method_bind* mb_set_rect;
    };

    static MethodBindings method_bindings;
    static void* class_tag;
};
} // namespace Rebel

#endif // REBEL_BACKBUFFERCOPY_H

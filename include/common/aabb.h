...
public:
    ...
    inline bool has_no_area() const {
        return (
            size.x <= CMP_EPSILON || size.y <= CMP_EPSILON
            || size.z <= CMP_EPSILON
        );
    }
    ...
    inline void set_position(const Vector3& p_position) {
        position = p_position;
    }

    inline const Vector3& get_size() const {
        return size;
    }

    inline void set_size(const Vector3& p_size) {
        size = p_size;
    }
    ...
    ); /** expand to contain a point if necesary */
